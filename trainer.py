...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(
        ...
        # 从training_args获取level_probs并确保batch_size匹配
        level_probs = None
        if hasattr(self.args, "level_probs"):
            level_probs = self.args.level_probs
            # 如果level_probs是单个样本的概率分布，需要扩展到整个batch
            if len(level_probs.shape) == 1:
                level_probs = level_probs.unsqueeze(0).expand(batch_size, -1)
            # 确保数据类型与模型一致
            if hasattr(model, "dtype"):
                level_probs = level_probs.to(dtype=model.dtype)
            #logger.info_rank0(f"处理后的level_probs shape: {level_probs.shape}, dtype: {level_probs.dtype}")
        else:
            logger.warning_rank0("training_args中没有level_probs")
        
        # 获取图像输入 - 使用pixel_values而不是images
        pixel_values = inputs.get("pixel_values")
        image_grid_thw = inputs.get("image_grid_thw")
        #logger.info_rank0(f"Pixel values type: {type(pixel_values)}")
        # if pixel_values is not None:
        #     logger.info_rank0(f"Pixel values shape: {pixel_values.shape if hasattr(pixel_values, 'shape') else 'no shape'}")
        #     logger.info_rank0(f"Pixel values dtype: {pixel_values.dtype if hasattr(pixel_values, 'dtype') else 'no dtype'}")
        # else:
        #     logger.warning_rank0("Pixel values is None - 这可能是问题所在！")
            
        # if image_grid_thw is not None:
        #     logger.info_rank0(f"Image grid thw: {image_grid_thw}")
        # else:
        #     logger.warning_rank0("Image grid thw is None - 这可能是问题所在！")
            
        # 检查其他可能的图像相关字段
        # for key in inputs.keys():
        #     if 'image' in key.lower() or 'pixel' in key.lower():
        #         logger.info_rank0(f"Found image-related key: {key}, value: {inputs[key]}")
        ...
        outputs = calculate_single_loss(
            ...
            level_probs=level_probs,  # 使用处理后的level_probs
